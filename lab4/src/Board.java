...
public class Board {
    ...
    public boolean move_figure(int row1, int col1, int row2, int col2 ){
        ...
        if (figure.canMove(row1, col1, row2, col2) && this.fields[row2][col2]==null){
            ...
            return true;
        } else if (figure.canAttack(row1, col1, row2, col2) && !this.fields[row2][col2].getName().equals("K") && this.fields[row2][col2] != null && this.fields[row2][col2].getColor() != this.fields[row1][col1].getColor() ){
    ...
    public boolean getCheck(int row, int col, int row1, int col1){
        Figure figure =  this.fields[row][col];

        if (figure.canAttack(row, col, row1, col1) && this.fields[row1][col1].getName().equals("K")){
            System.out.println("check");
            this.fields[row1][col1].setCheckKing("check");
            return true;
        }
    ...
    public boolean getCheckmate(int row, int col, int row1, int col1){
        Figure king =  this.fields[row1][col1];

        if (this.getCheck(row, col, row1, col1) && king.canMove()
        {
            System.out.println("check");
            return true;
        }
